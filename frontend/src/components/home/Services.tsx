      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="mt-16 grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((s) => (
            <div
              key={s.name}
              className="rounded-xl bg-white p-8 shadow-sm border border-brand-grey-1000 hover:border-brand-purple-300 transition-colors"
            >
              <h3 className="font-heading text-lg font-medium text-brand-purple-1000">
                {s.name}
              </h3>
              <p className="mt-4 font-body text-sm text-gray-600 leading-relaxed">
                {s.desc}
              </p>
            </div>
          ))}
